...
function copy(x::ArfFloat{P}) where {P}
    ...
    ccall(@libarb(arf_set), Cvoid, (Ref{ArfFloat{P}}, Ref{ArfFloat{P}}), z, x)
    return z
end
...
function initializer(::Type{ArfFloat{P}}) where {P}
    ...
    ccall(@libarb(arf_init), Cvoid, (Ref{ArfFloat{P}}, ), z)
    finalizer(release, z)
    return z
end
...
function zero(::Type{ArfFloat{P}}) where {P}
    ...
    ccall(@libarb(arf_zero), Cvoid, (Ref{ArfFloat{P}}, ), z)
    return z
end

function one(::Type{ArfFloat{P}}) where {P}
    z = initializer( ArbFloat{P} )
    ccall(@libarb(arf_one), Cvoid, (Ref{ArfFloat{P}}, ), z)
    return z
end


function isnan(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_nan), Cint, (Ref{ArfFloat},), x)
end
function isinf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_inf), Cint, (Ref{ArfFloat},), x)
end
function isposinf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_pos_inf), Cint, (Ref{ArfFloat},), x)
end
function isneginf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_neg_inf), Cint, (Ref{ArfFloat},), x)
end
function iszero(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_zero), Cint, (Ref{ArfFloat},), x)
end
function isone(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_one), Cint, (Ref{ArfFloat},), x)
end
function isfinite(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_finite), Cint, (Ref{ArfFloat},), x)
end
function isnormal(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_normal), Cint, (Ref{ArfFloat},), x)
end
function isspecial(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_special), Cint, (Ref{ArfFloat},), x)
end
...
function convert(::Type{BigFloat}, x::ArfFloat{P}) where {P}
    z = BigFloat(0)
    r = ccall(@libarb(arf_get_mpfr), Int, (Ref{BigFloat}, Ref{ArfFloat{P}}, Cint), z, x, 0)
    return z
end

function convert(::Type{ArfFloat{P}}, x::BigFloat) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_mpfr), Cvoid, (Ref{ArfFloat{P}}, Ref{BigFloat}), z, x)
    z
end
...
function convert(::Type{ArfFloat{P}}, x::Int64) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_si), Cvoid, (Ref{ArfFloat{P}}, Ref{Int64}), z, x)
    z
end
...
function convert(::Type{ArfFloat{P}}, x::Float64) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_d), Cvoid, (Ref{ArfFloat{P}}, Ref{Float64}), z, x)
    z
end
...
function round(x::T, prec::Int64) where {T <: ArfFloat}
    P = precision(T)
    z = ArfFloat{P}()
    ccall(@libarb(arf_set_round), Int, (Ref{ArfFloat}, Ref{ArfFloat}, Int64, Int), z, x, prec, 2)
    return z
end
...
function frexp{P}(x::ArfFloat{P})
   ...
   ccall(@libarb(arf_frexp), Void, (Ref{ArfFloat{P}}, Int64, Ref{ArfFloat{P}}), &significand, exponentOf2, x)
   significand, exponentOf2
end
=#

function min(x::T, y::T) where {T <: ArfFloat}
    c = ccall(@libarb(arf_cmp), Cint, (Ref{ArfFloat}, Ref{ArfFloat}), x, y)
    return (c < 0) ? x : y
end
