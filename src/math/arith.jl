...
function abs(x::ArbFloat{P}) where {P}
    ...
    ccall(@libarb(arb_get_abs_lbound_arf), Cvoid, (Ref{ArfFloat{P}}, Ref{ArbFloat{P}}, Int), lo, x, P) 
    ccall(@libarb(arb_get_abs_ubound_arf), Cvoid, (Ref{ArfFloat{P}}, Ref{ArbFloat{P}}, Int), hi, x, P)
    ccall(@libarb(arb_set_interval_arf), Cvoid, (Ref{ArbFloat{P}}, Ref{ArfFloat{P}}, Ref{ArfFloat{P}}, Int), z, lo, hi, P)
    return z
end
...
for (op,cfunc) in ((:-,:arb_neg), (:sign, :arb_sgn), (:absz, :arb_abs))
  @eval begin
    function ($op)(x::ArbFloat{P}) where {P}
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Cvoid, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}), z, x)
      return z
    end
  end
end
...
function (*)(x::ArbFloat{P}, y::ArbFloat{P}) where {P}
    z = initializer(ArbFloat{P})
    ccall(@libarb(arb_mul), Cvoid, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Int), z, x, y, P)
    return z
end
...
for (op,cfunc) in ((:+,:arb_add_si), (:-, :arb_sub_si), (:*, :arb_mul_si), (:/, :arb_div_si))
  @eval begin
    function ($op)(x::ArbFloat{P}, y::Int) where {P}
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Cvoid, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Int, Int), z, x, y, P)
      return z
    end
  end
end
...
for (op,cfunc) in ((:addmul,:arb_addmul), (:submul, :arb_submul))
  @eval begin
    function ($op)(w::ArbFloat{P}, x::ArbFloat{P}, y::ArbFloat{P}) where {P}
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Cvoid, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Int), z, w, x, y, P)
      z
    end
  end
end
