...
function logbase(x::ArbFloat{P}, base::Int) where {P}
    b = UInt(abs(base))
    z = initializer(ArbFloat{P})
    ccall(@libarb(arb_log_base_ui), Void, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, UInt, Int), z, x, b, P)
    z
end
...
for (op,cfunc) in ((:root, :arb_root_ui),)
  @eval begin
    function ($op)(x::ArbFloat{P}, y::UInt) where {P}
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Void, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, UInt, Int), z, x, y, P)
      z
    end
  end
end
...
for (op,cfunc) in ((:^,:arb_pow), (:pow,:arb_pow))
  @eval begin
    function ($op)(x::I, y::ArbFloat{P}) where {P,I <: Integer}
      xx = ArbFloat{P}(x)
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Void, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Int), z, xx, y, P)
      return z
    end
    function ($op)(x::ArbFloat{P}, y::I) where {P,I <: Integer}
      ...
      yy = ArbFloat{P}(ay)
      z = initializer(ArbFloat{P})
      ccall(@libarb($cfunc), Void, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, Int), z, x, yy, P)
      return sy ? inv(z) : z
    end
...
function root(x::ArbFloat{P}, y::Integer) where {P}
   return (
     if y>=0
       yy = UInt64(y)
      z = initializer(ArbFloat{P})
       ccall(@libarb(arb_root_ui), Void, (Ref{ArbFloat{P}}, Ref{ArbFloat{P}}, UInt64, Int), z, x, yy, P)
       z
    else
