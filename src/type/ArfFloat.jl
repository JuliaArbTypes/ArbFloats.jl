...
function zero(::Type{ArfFloat{P}}) where {P}
    ...
    ccall(@libarb(arf_zero), Void, (Ptr{ArfFloat{P}}, ), Ref{z})
    return z
end

function one(::Type{ArfFloat{P}}) where {P}
    z = initializer( ArbFloat{P} )
    ccall(@libarb(arf_one), Void, (Ptr{ArfFloat{P}}, ), Ref{z})
    return z
end


function isnan(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_nan), Cint, (Ptr{T},), Ref{x})
end
function isinf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_inf), Cint, (Ptr{T},), Ref{x})
end
function isposinf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_pos_inf), Cint, (Ptr{T},), Ref{x})
end
function isneginf(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_neg_inf), Cint, (Ptr{T},), Ref{x})
end
function iszero(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_zero), Cint, (Ptr{T},), Ref{x})
end
function isone(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_one), Cint, (Ptr{T},), Ref{x})
end
function isfinite(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_finite), Cint, (Ptr{T},), Ref{x})
end
function isnormal(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_normal), Cint, (Ptr{T},), Ref{x})
end
function isspecial(x::T) where {T <: ArfFloat}
    zero(Cint) != ccall(@libarb(arf_is_special), Cint, (Ptr{T},), Ref{x})
end
...
function convert(::Type{BigFloat}, x::ArfFloat{P}) where {P}
    z = BigFloat(0)
    r = ccall(@libarb(arf_get_mpfr), Int, (Ptr{BigFloat}, Ptr{ArfFloat{P}}, Cint), Ref{z}, Ref{x}, 0)
    return z
end

function convert(::Type{ArfFloat{P}}, x::BigFloat) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_mpfr), Void, (Ptr{ArfFloat{P}}, Ptr{BigFloat}), Ref{z}, Ref{x})
    z
end
...
function convert(::Type{ArfFloat{P}}, x::Int64) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_si), Void, (Ptr{ArfFloat{P}}, Ptr{Int64}), Ref{z}, Ref{x})
    z
end
...
function convert(::Type{ArfFloat{P}}, x::Float64) where {P}
    z = initializer(ArfFloat{P})
    ccall(@libarb(arf_set_d), Void, (Ptr{ArfFloat{P}}, Ptr{Float64}), Ref{z}, Ref{x})
    z
end
...
function round(x::T, prec::Int64) where {T <: ArfFloat}
    P = precision(T)
    z = ArfFloat{P}()
    ccall(@libarb(arf_set_round), Int, (Ptr{T}, Ptr{T}, Int64, Int), Ref{z}, Ref{x}, prec, 2)
    return z
end
...
function frexp{P}(x::ArfFloat{P})
   ...
   ccall(@libarb(arf_frexp), Void, (Ptr{ArfFloat{P}}, Int64, Ptr{ArfFloat{P}}), Ref{s}ignificand, exponentOf2, Ref{x})
   significand, exponentOf2
end
=#

function min(x::T, y::T) where {T <: ArfFloat}
    c = ccall(@libarb(arf_cmp), Cint, (Ptr{T}, Ptr{T}), Ref{x}, Ref{y})
    return (c < 0) ? x : y
end
